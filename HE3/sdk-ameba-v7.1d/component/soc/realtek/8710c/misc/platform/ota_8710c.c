...
/******************************************** */
int Erase_OTA_Region(unsigned int Address)
{
	int write_sector = 0;
	int erased = 0;
	unsigned int adress = Address;
	printf("Erasing OTA partition\n");
	printf("%s : Erase Start Address:%08x\n",__FUNCTION__ ,adress);
	while (erased < 0x001C4000) //1808kb
	{
...
/***************************shaik************** */
#if 0
int write_to_flash(unsigned int address, int len, unsigned char buff)
{
	//device_mutex_lock(RT_DEV_LOCK_FLASH);
	if (flash_burst_write(&flash_ota, address, len, buff) < 0)
	{
		printf("\n\r[%s] Write stream failed", __FUNCTION__);
		//device_mutex_unlock(RT_DEV_LOCK_FLASH);
		return 1;
...
void read_from_flash(unsigned int address, int len, unsigned char buff)
{

	//device_mutex_lock(RT_DEV_LOCK_FLASH);
	if (flash_burst_read(&flash_ota, address, len, buff) < 0)
	{
		printf("\n\r[%s] Read stream failed", __FUNCTION__);
	//	device_mutex_unlock(RT_DEV_LOCK_FLASH);
		return 1;
	}
	//device_mutex_unlock(RT_DEV_LOCK_FLASH);
