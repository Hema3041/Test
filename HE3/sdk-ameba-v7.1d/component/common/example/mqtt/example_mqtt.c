...
void prvMQTTEchoTask(void *pvParameters)
{
	...
	char* address = "a198fgj6igmrrt-ats.iot.ap-south-1.amazonaws.com";//"gpssensor.ddns.net";
	...
	//printf("MQTTDisconnect Status : %d\n",MQTTDisconnect(&client));
	...
	while (1)
	{
		...
		if ((rc = MQTTPublish(&client, pub_topic, &message)) != 0)
		{
			...
			printf("Published Success: hello from Shaik\n");
		}
...
{	
	...
	if(c->mqttstatus == MQTT_RUNNING){
		count++;
		sprintf(payload, "hello from Mohammd Shaik %d", count);
		message.payloadlen = strlen(payload);			
...
static void prvMQTTTask(void *pvParameters)//Not in use
{
	/* To avoid gcc warnings */
	( void ) pvParameters;
