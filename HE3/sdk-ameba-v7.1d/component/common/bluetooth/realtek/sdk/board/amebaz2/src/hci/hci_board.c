...
bool MAC_Id_Read(void)
{
    char efuse_buf[6];
    memset(efuse_buf, 0x00, sizeof(efuse_buf));

    efuse_logical_read(BOARD_LOGIC_EFUSE_OFFSET, 6, efuse_buf);
    ...
    {                    
        ...
        for(int i = 0 ;i <6;i ++)
        {
            printf("%02x:",efuse_buf[i]);
        }
        printf("\r\n");
    }
    else
    {
        printf("*********** EFUSE MAC ID NOT YET WRITE ***********\n");
    }
    return 1;
}
            ...
            if (entry_len == 6)
            {
                ...
                {                    
                   // memcpy(p,&efuse_buf[0],6);
                    ...
                    //Hoags' implementation start
                    extern struct univMacid umObj;
                    memset(umObj.macid, 0, sizeof(umObj.macid));
                    #if 0 //Suvarna: Commented temporarily
                    sprintf(umObj.macid, "%02x%02x%02x%02x%02x%02x",
                    efuse_buf[0], efuse_buf[1], efuse_buf[2], efuse_buf[3], efuse_buf[4], efuse_buf[5]);
                    #else
                    strcpy(umObj.macid, "aabbccddeeee");
                    #endif
