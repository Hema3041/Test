...
int print_help_handler(char *cmd){
	...
	else if(strcmp(cmd, "hstat cpu") == 0){
		fATSS();
		return 0;
	}
	else if(strcmp(cmd , "hstat ram") == 0){
	...
	else if(strcmp(cmd, "reboot uartburn") == 0){
		printf("This command is not implemented yet!!!\n");
		return 0;
	}
	else if(strcmp(cmd, "henv -a") == 0){
		displayEnvInRAM();
		return 0;
	}
	else if(strcmp(cmd, "hreset") == 0){
		factoryReset();
		return 0;
	}
	else if(strcmp(cmd, "hlist") == 0){	//file in dir 
		printf("List of File:-\n");
		//hlist();
		return 0;
	}
	else if(strcmp(cmd, "hflash read") == 0){	 
		printf("Before Calling hflash Read\n");
		printStoredData();
		return 0;
	}
...
#define STACKSIZE               1024	//Suvarna: Don't modify this as it causes wifi to not work properly
void start_log_service(void)
{
