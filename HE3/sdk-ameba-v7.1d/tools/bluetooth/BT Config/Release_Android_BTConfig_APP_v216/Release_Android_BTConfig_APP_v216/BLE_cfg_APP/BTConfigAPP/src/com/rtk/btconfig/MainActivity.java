...
public class MainActivity extends Activity 
{
	                ...
	                case MotionEvent.ACTION_UP:
	                	...
	                	if(rect.contains(v.getLeft() + (int) event.getX(), v.getTop() + (int) event.getY())){
	                		...
	                		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)  {
	                			...
	                			if(!hasPermissions(mMainAct, PERMISSIONS)){
                				...
                				if(!hasPermissions(mMainAct, PERMISSIONS)){
                        			...
                        			mWlanScanListView_2G.setOnItemClickListener(new AdapterView.OnItemClickListener(){
                        				...
                        				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        					showAPConnectDialog((byte)0, position, 1);
                        				}               				
                        			});
        ...
        switch (requestCode) {
            case PERMISSION_ALL: {
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED && grantResults[1] == PackageManager.PERMISSION_GRANTED){
                    ...
                    .setMessage("Go to Settings and Grant the permissions to use this feature.")
	...
	public void scan_BT_peripheral() 
	{
		...
		deviceList_builder.setTitle("Choose device to configure");
		...
		for(int i = 0; i < tmpScanResults.size(); i++){
			...
			if(!tmpScanResults.get(i).getMac().equals("00:00:00:00:00:00")){			
				if(!mWlanScanObjs_2G.contains(tmpScanResults.get(i))){
					...
					mWlanScanObjs_2G.get(mWlanScanObjs_2G.indexOf(tmpScanResults.get(i))).setEncrpytType(tmpScanResults.get(i).getEncrpytType());	
				}		
			}				
		}
	...
	protected void updateWlan2GScanUI(){
		...
		for(int i = 0; i < tmpScanResults.size(); i++){
			...
			if(!tmpScanResults.get(i).getMac().equals("00:00:00:00:00:00")){
				...
				if(!mWlanScanObjs_2G.contains(tmpScanResults.get(i))){
					mWlanScanObjs_2G.add(tmpScanResults.get(i));					
					mWlanScanObjs_2G.get(mWlanScanObjs_2G.indexOf(tmpScanResults.get(i))).setBand((byte) 0);
				}
				else{
					// update TTL and RSSI and security type for existed AP
					mWlanScanObjs_2G.get(mWlanScanObjs_2G.indexOf(tmpScanResults.get(i))).resetTTL(BTConfig.mTTL_count);				
    	...
    	if (mCheckBTConfigStateTimer.wlan2GEnabled == 1 && mCheckBTConfigStateTimer.wlan5GEnabled == 1){
        	...
        	for(int i = 0; i < tmpScanResults.size(); i++){
    			...
    			if(!tmpScanResults.get(i).getMac().equals("00:00:00:00:00:00")){
    				...
    				if(!mWlanScanObjs_2G.contains(tmpScanResults.get(i))){
    					mWlanScanObjs_2G.add(tmpScanResults.get(i));					
						mWlanScanObjs_2G.get(mWlanScanObjs_2G.indexOf(tmpScanResults.get(i))).setBand((byte) 1);
    				}
    				else{
        	...
        	for(int i = 0; i < tmpScanResults.size(); i++){
    			...
    			if(!tmpScanResults.get(i).getMac().equals("00:00:00:00:00:00")){
    				...
    				if(!mWlanScanObjs_5G.contains(tmpScanResults.get(i))){
						...
						mWlanScanObjs_5G.get(mWlanScanObjs_5G.indexOf(tmpScanResults.get(i))).setBand((byte) 1); 
    				}
    				else{
    					// update TTL and RSSI for existed AP
    					...
    					mWlanScanObjs_5G.get(mWlanScanObjs_5G.indexOf(tmpScanResults.get(i))).setEncrpytType(tmpScanResults.get(i).getEncrpytType());	
    				}		
    			}				
    		}
	...
	private void judgeConnectStatus(String ssid){
		...
		else if(tmpConnectStatus == 0x02 || tmpConnectStatus == 0x05){ /* connecting */
		}
		else{ /* connect fail */
					...
					public void run() {
						...
						builder.setPositiveButton("OK", new DialogInterface.OnClickListener(){
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								else if(msg.equals("Connection to AP Timeout")){
											if(mCheckBTConfigStateTimer.wlan2GEnabled == 0 && mCheckBTConfigStateTimer.wlan5GEnabled == 1){
												mBTConfig.setBTConfigState(BTConfigState.STATE_BT_SCAN_WLAN_5G_END);
												mWlanScanListView_5G.setVisibility(View.VISIBLE);
											}else{
